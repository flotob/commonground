...
const ConnectAeternityWalletButton: React.FC<Props> = (props) => {
  ...
  const linkCurrentWallet = useCallback(async () => {
    if (!!address) {
      try {
        ...
        setWalletData({ type: 'aeternity', data, signature });
        setError(undefined);
      } catch (e) {
  ...
  }, [address, chainId, signMessageAsync, setWalletData]);
          ...
          {(() => {
            if (!connected) {
              return (<Button
                ...
                text='Connect Aeternity wallet'
                iconRight={<AeternityIcon className='w-5 h-5' />}
...
export default React.memo(ConnectAeternityWalletButton);