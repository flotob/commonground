...
const SignWalletPageAeternity: React.FC<Props> = (props) => {
  ...
  const { linkCurrentWallet: linkAeternityWallet, address, isConnected } = useAeternityWallet();

  const isActiveAddressLinked = useMemo(() => {
    return !!wallets?.find(wallet => wallet.walletIdentifier === address && wallet.type === 'aeternity');
  }, [address, wallets]);

  const linkCurrentWallet = useCallback(async () => {
    if (!isActiveAddressLinked && !!address) {
      try {
        const aeternityResult = await linkAeternityWallet();
        
        if (aeternityResult) {
          const { data, signature } = aeternityResult;
  ...
  }, [isActiveAddressLinked, address, linkAeternityWallet, showSnackbar]);

  // Redirect to wallet page when wallet is connected
  if (enableWalletRedirect.current && isActiveAddressLinked) {
    const wallet = wallets?.find(wallet => wallet.walletIdentifier === address && wallet.type === 'aeternity');
    if (wallet) {
...
export default React.memo(SignWalletPageAeternity);