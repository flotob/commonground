...
if (isMainThread) {
  throw new Error("calculateTokenRewardProgramSecond can only be run as a worker job");
}

const ONESHOT_ID = '2024_12_11_calculate_token_reward_program_second';
...
(async () => {
    ...
    if (!oneshots.find(d => d.id === ONESHOT_ID)) {
            ...
            if (!!userId) {
                try {
                    ...
                    if (result.rows.length > 0) {
                        ...
                        if (!rewardEntry.oldAccountWithMessageReward && !rewardEntry.oldAccountWithoutMessageReward) {
                            throw new Error("No reward entry found for user " + userId);
                        }
                        existingRewardProgram.totalReward = new Decimal(existingRewardProgram.totalReward!).plus(new Decimal(rewardEntry.totalReward!)).toFixed(2);
                        await pool.query(`
        ...
        console.log(`=== Finished generating second token reward program amounts after ${dur}s ===`);
        console.log("=== Updated", updatedUsers, "users, inserted", insertedUsers, "users ===");
    }
    else {
        console.log("=== Second token reward program amounts were already generated, nothing to do ===");
    }
})();